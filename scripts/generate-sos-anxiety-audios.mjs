/**
 * Gera MP3 SOS ansiedade via ElevenLabs + meta JSON para sessions.ts
 * Uso: node scripts/generate-sos-anxiety-audios.mjs [id opcional]
 */
...
const resolvedDir = path.join(__dirname, 'resolved-sos-anxiety');
...
const MARKER_RE = /\{\{BIBLE:([A-Za-z0-9_-]+)\}\}/g;
...
function formatBiblical(passageId) {
  ...
  if (!passage?.text?.trim() || !passage.reference || !passage.verses?.length) {
    throw new Error(
      `Passagem bíblica ausente: ${passageId}. Rode npm run fetch:prayers.`,
    );
  }
...
async function generateOne(id, entry, text) {
  ...
  console.log(
    `  Voz: ${voiceId} | speed=${settings.speed} stability=${settings.stability}`,
  );
...
for (const id of ids) {
  ...
  const outPath = await generateOne(id, entry, resolved);
  const realDuration = await probeDurationSeconds(outPath);
  ...
  meta[id] = {
    ...
    biblicalPrayerIds: entry.biblicalPrayerIds,
    ambientKey: entry.ambientKey,
    ambientVolume: entry.ambientVolume,
    durationSeconds: realDuration ?? entry.durationSeconds ?? 300,
  };
}

if (!onlyId) {
  fs.writeFileSync(metaPath, JSON.stringify(meta, null, 2), 'utf8');
  console.log(`\nMeta salva em ${metaPath}`);
} else {
  // merge single into existing meta if present
  let existing = {};
  ...
  console.log(`\nMeta atualizada em ${metaPath}`);
}

console.log('\nÁudios SOS ansiedade gerados com sucesso.');
