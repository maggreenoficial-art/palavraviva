...
const scripts = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'meditation-scripts.json'), 'utf8'),
);

const outDir = path.join(root, 'assets', 'audio');
const resolvedDir = path.join(__dirname, 'resolved-meditation');
...
for (const id of ids) {
  ...
  if (words < 220) {
    console.warn(`  Aviso: roteiro curto (~${words} palavras). Meta ≈ 300.`);
  }
  if (words > 380) {
    console.warn(`  Aviso: roteiro longo (~${words} palavras). Pode passar de 3 min.`);
  }

  await generateOne(id, entry, resolved);
}

console.log('\nÁudios de meditação gerados com sucesso.');
