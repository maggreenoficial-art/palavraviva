...
const scripts = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'premium-series-scripts.json'), 'utf8'),
);

const outDir = path.join(root, 'assets', 'audio');
const resolvedDir = path.join(__dirname, 'resolved-premium');
...
function formatBiblical(passageId) {
  ...
  if (!passage?.text?.trim() || !passage.reference || !passage.verses?.length) {
    throw new Error(
      `Passagem bÃ­blica ausente no dataset: ${passageId}. Rode npm run fetch:prayers. NÃ£o invente texto.`,
    );
  }
...
function resolveScript(script) {
  ...
  if (leftover?.length) {
    throw new Error(`Marcadores nÃ£o resolvidos: ${leftover.join(', ')}`);
  }
...
async function generateOne(id, entry, text) {
  ...
  console.log(`  âœ“ Salvo: ${outPath} (${(buffer.length / 1024).toFixed(1)} KB)`);
}
...
for (const id of ids) {
  ...
  if (!entry) {
    console.error(`Script nÃ£o encontrado: ${id}`);
    process.exit(1);
  }
  ...
  console.log(`Roteiro resolvido (${used.join(', ')}) â†’ ${resolvedPath}`);

  const words = countWords(resolved);
  const breaks = sumBreakSeconds(resolved);
  const estMin = words / 120 + breaks / 60;
  console.log(
    `  Estimativa â‰ˆ ${estMin.toFixed(1)} min (palavras=${words}, pausas=${breaks.toFixed(1)}s)`,
  );
  if (words < 520) {
...
console.log('\nÃudios da sÃ©rie ecossistema gerados com sucesso.');
