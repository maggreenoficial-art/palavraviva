...
import { dataDir as resolveDataDir } from './payments-shared.mjs';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const root = path.resolve(__dirname, '..');
const storageDir = resolveDataDir();
const eventsPath = path.join(storageDir, 'events.json');
const usersPath = path.join(storageDir, 'users.json');
const subscriptionsPath = path.join(storageDir, 'subscriptions.json');
const checkoutsPath = path.join(storageDir, 'checkouts.json');
const generationsPath = path.join(storageDir, 'foto-jesus-generations.json');
      ...
      app.innerHTML = \`
        ...
        <div class="wrap">
          ...
          <div class="card" style="margin-top:14px">
            ...
            <p class="section-note">Dados em tempo real do site em produção. Atualiza a cada 12s.</p>
...
export async function ingestAnalyticsEvent(body, req) {
  ...
  if (
    saved.userId &&
    (saved.name === 'subscription_activated' || saved.name === 'subscription_start')
  ) {
    ingestSubscriptionFromEvent(saved);
  }
  if (
    saved.userId &&
    (saved.name === 'tool_purchase_activated' || saved.name === 'foto_jesus_success')
  ) {
...
export function handleAdminSubscriptionAction(body) {
  ...
  if (!subscription) {
    throw new Error('assinatura_nao_encontrada');
  }
...
export function syncAnalyticsPayload(body) {
